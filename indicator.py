...
def macd(frames):
    ...
    df['ema '+str(e1)] = calculator.ema(df,e1)
    df['ema '+str(e2)] = calculator.ema(df,e2)
    df['ema '+str(e1)+'-'+str(e2)] = df['ema '+str(e1)] - df['ema '+str(e2)] 
    a = calculator.ema_2(df['ema '+str(e1)+'-'+str(e2)] , e3, df['ma'])
    print(a)
    df['ema '+str(e3)] = a
    #save
...
def sma_ema(frames):
    ...
    df['moving average '+str(p3)] = calculator.ema(df,p3)
    #save
